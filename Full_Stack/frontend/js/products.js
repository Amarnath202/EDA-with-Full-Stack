// Helper function to get correct image filename for products
function getProductImageFilename(productName) {
    // Special case for Organic Products
    if (productName.trim().toLowerCase() === 'organic products') {
        return 'Organic Products.jpg';
    }
    // Add more special cases if needed
    // Default: lowercase, replace spaces with hyphens, use .webp
    return productName.trim().toLowerCase().replace(/\s+/g, '-') + '.webp';
}
...
class ProductsManager {
        ...
        productsGrid.innerHTML = products.map(product => `
            <div class="product-card">
                <div class="product-image">
                        <img src="./img/${getProductImageFilename(product.name)}" alt="${product.name}" onerror="this.src='./img/no-image.png'">
    ...
    formatCategory(category) {
        ...
        return map[category] || (category ? category.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ') : '');
    }
